...
export class ReporteTurnosComponent implements OnInit {
  ...
  readonly fechaDesdeFiltro = signal('');
  readonly fechaHastaFiltro = signal('');

  readonly turnosFiltrados = computed(() => {
    const peluqueroFiltro = this.peluqueroFiltro();
    const fechaDesde = this.fechaDesdeFiltro();
    const fechaHasta = this.fechaHastaFiltro();
    const todos = this.turnoService.turnos();

    return todos.filter((turno) => {
      const coincidePeluquero =
        peluqueroFiltro === null || turno.peluqueroId === peluqueroFiltro;
      const coincideDesde = !fechaDesde || turno.fecha >= fechaDesde;
      const coincideHasta = !fechaHasta || turno.fecha <= fechaHasta;
      return coincidePeluquero && coincideDesde && coincideHasta;
    });
  });
  ...
  actualizarFechaDesde(event: Event): void {
    this.fechaDesdeFiltro.set((event.target as HTMLInputElement).value);
  }

  actualizarFechaHasta(event: Event): void {
    this.fechaHastaFiltro.set((event.target as HTMLInputElement).value);
  }
}
