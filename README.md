...
- **El backend debe estar corriendo en `http://localhost:8080`**. Consultá el README del repositorio del backend para su instalación y ejecución.
...
Abre automáticamente `http://localhost:4200` en el navegador. La aplicación consume la API REST en `http://localhost:8080/api` — asegurate de que el backend esté corriendo antes de iniciar el frontend.
...
│   └── environment.prod.ts         # { production: true, apiUrl: 'http://localhost:8080/api' }
...
El backend tiene habilitado CORS para el origen `http://localhost:4200`, por lo que el frontend consume la API directamente en `http://localhost:8080/api` sin necesidad de configurar un proxy en Angular.
